...
The ~extractelement~ is not compile correctly. It uses ~srli~ to shift the value in the byte with the offset provided by ~extractelement~, instead of actually calculating the offset. See this [[./clang-bugs/extract-element-disagreement][directory]], which includes a bug report summary. This bug is reported in Clang 13 but is optimized away in Clang 19.
...
*** extractvalue a packed struct with non-2-powered sized vector
The offset for ~extractvalue~ from a packed struct with a vector that has size not being a power of 2 is calculated incorrectly. This is mainly because the function for calculating the size of the vector does not know whether the struct is "packed" or not. This bug can be found in this [[./clang-bugs/extractvalue-packedstruct-vector][directory]]. The relevant bug report was submitted and recognized on [[https://github.com/llvm/llvm-project/issues/124061][github]]
** Generator issues
*** use-after-free
This is an issue with our generator in the malloc branch. More specifically, this is caused by us storing a value into a pointer that is freed. Because the pointer is an alias of the original freed pointer (through various instructions), we were not able to determine whether this pointer is freed or not. A solution is to use a union-find technique to tag each pointer with how it is generated (or how its ancestor is generated) to determine whether they can be safely used.ls
