...
# Tracing
Besides regular interpretation using `vellvm`, one can also output the trace of a program. In particular:
- `src/vellvm -i -log tests/ll/factorial.ll` to run and output the log of the trace of a program to `stdout`.
- `src/vellvm -t tests/ll/factorial.ll` to run and output the trace (which is located at the main function) on a given file into `factorial.base.trace` in the output directory.

The trace is a psudo-LLVM format that can be further processed using the following command: 
- `src/vellvm -trace executable tests/ll/factorial.base.trace` can compiled a base trace into a standard `.ll` format file that can be interpreted by `vellvm` or compiled using `clang` into executable. In other words, this command allows one to interpret the trace (in most case, usually as long as the trace is generated by `-t` without modification).
- `src/vellvm -trace main tests/ll/factorial.xxx` will only output the main function to `factorial.main.trace`.

The trace will stop at approximately the point when the interpreter fails, or the full interpreted result. 
