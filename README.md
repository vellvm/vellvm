...
The trace is a pseudo-LLVM format that can be further processed using the following command: 
- `src/vellvm -trace executable tests/ll/factorial.base.trace` can compile a base trace into a standard `.ll` format file that can be interpreted by `vellvm` or compiled using `clang` into the executable. In other words, this command allows one to interpret the trace generated by `src/vellvm -t`.
- `src/vellvm -trace main tests/ll/factorial.xxx` will only output the main function to `factorial.main.trace`.

The trace will stop at approximately the point when the interpreter fails or when the full interpreted result is reached. 
