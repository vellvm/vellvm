...
let llc opt_level = Printf.sprintf "llc %s -o " opt_level
...
let clang_compile (dot_ll : string) (dot_s : string) (opt_level : string) :
    unit =
...
let link (mods : string list) (out_fn : string) (opt_level : string) : unit =
  sh
    (sprintf "%s%s %s %s %s %s" (link_cmd opt_level) out_fn
