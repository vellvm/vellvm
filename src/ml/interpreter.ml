  ...
  fun ppf -> function
    ...
    | UVALUE_I1 x ->
        fprintf ppf "UVALUE_I1(%d)"
          (Camlcoq.Z.to_int (TwoPhaseIntegers.Int1.unsigned x))
    | UVALUE_I8 x ->
        fprintf ppf "UVALUE_I8(%d)"
          (Camlcoq.Z.to_int (TwoPhaseIntegers.Int8.unsigned x))
    | UVALUE_I32 x ->
        fprintf ppf "UVALUE_I32(%d)"
          (Camlcoq.Z.to_int (TwoPhaseIntegers.Int32.unsigned x))
    | UVALUE_I64 x ->
        fprintf ppf "UVALUE_I64(%s)"
          (Int64.to_string (Z.to_int64 (TwoPhaseIntegers.Int64.unsigned x)))
    | UVALUE_IPTR x ->
...
let char_of_I8 x =
  char_of_int (Camlcoq.Z.to_int (TwoPhaseIntegers.Int8.unsigned x))

(* Converts a list of TwoPhaseIntegers.Int8 values to OCaml string *)
let string_of_bytes (bytes : TwoPhaseIntegers.Int8.int list) : bytes =
 ...
 *   step (k (Obj.magic (DV.DVALUE_I64 TwoPhaseIntegers.Int64.zero))) *)
