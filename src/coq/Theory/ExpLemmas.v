...
Section Translations.
  ...
  Lemma LU_to_exp_Global : forall {X} (e : LLVMGEnvE X),
    subevent X (F := exp_E) (LU_to_exp (subevent X e)) = subevent X e.
  Proof.
    reflexivity.
  Qed.

  Lemma exp_to_instr_Global : forall {X} (e : LLVMGEnvE X),
      subevent X (F := instr_E) (exp_to_instr (subevent X e)) = subevent X e.
  Proof.
    reflexivity.
  Qed.
  ...
  Lemma FUB_to_exp_UB : forall {X} (e : UBE X),
      subevent X (F := instr_E) (FUB_to_exp (subevent X e)) = subevent X e.
  Proof.
    reflexivity.
  Qed.
