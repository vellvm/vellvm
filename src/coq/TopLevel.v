...
Definition allocate_global (g:global dtyp) : LLVM _CFG unit :=
  (vis (Alloca (g_typ _ g)) (fun v => trigger (GlobalWrite (g_ident _ g) (dvalue_to_uvalue v)))).
...
Definition allocate_declaration (d:declaration dtyp) : LLVM _CFG unit :=
  (* SAZ TODO:  Don't allocate pointers for LLVM intrinsics declarations *)
    vis (Alloca DTYPE_Pointer) (fun v => trigger (GlobalWrite (dc_name _ d) (dvalue_to_uvalue v))).
...
Definition global_env := FMapAList.alist raw_id uvalue.
...
Definition run_with_memory' (prog: list (toplevel_entity typ (list (block typ)))) :
  option (LLVM _MCFG3 (M.memory * ((local_env * stack) * (global_env * uvalue)))) :=
    ...
    let core_trace : LLVM _CFG uvalue :=
        ...
        (* CB TODO: This feels wrong. *)
        'daddr <- trigger (pick addr True) ;;
        D.denote_mcfg defns DTYPE_Void daddr main_args
    in
