...
Qed.
  ...
  (* Lemma dvalue_bytes_fin_to_dvalue_oom_fin_inf : *)
  (*   forall dt dvbs_fin dvbs_inf msg, *)
  (*     dvalue_bytes_refine dvbs_inf dvbs_fin -> *)
  (*       (@dvalue_bytes_to_dvalue ErrOOMPoison *)
  (*        (@EitherMonad.Monad_eitherT ERR_MESSAGE *)
  (*           (OomableT Poisonable) *)
  ...
  (*                   MonadPoisonable)) *)
  (*             (@RAISE_OOMABLE_OomableT Poisonable *)
  (*                MonadPoisonable)) dvbs_inf dt) = raise_oomable msg. *)
  (* Proof. *)
  (*   induction dt; *)
  (*     intros dvbs_fin dvbs_inf msg REF FIN. *)

  (*   { unfold dvalue_to_dvalue_bytes in FIN. *)
  (*     rewrite dvalue_bytes_to_dvalue_equation in FIN. *)
  (*     cbn in FIN. *)
  (*     remember (map_monad dvalue_byte_value dvbs_fin) as res. *)
  (*     destruct_err_oom_poison res; inv FIN. *)
  (*     - repeat break_match_hyp_inv. *)
  (*     - unfold dvalue_byte_value in *. *)
  (*       induction REF. *)
  (*       + inv Heqres. *)
  (*       + rewrite map_monad_unfold in Heqres. *)
  (*         destruct x, y. *)
  (*         cbn in H. *)
  (*         destruct H as (?&?&?); subst. *)
  (*         remember (dvalue_extract_byte dv0 dt0 (Z.of_N idx0)) as extr. *)
  (*         destruct_err_oom_poison extr; inv Heqres. *)
  (*         * cbn in *. *)
  (*           remember (map_monad *)
  (*                (fun x0 : dvalue_byte => *)
  (*                 match x0 with *)
  (*                 | DVALUE_ExtractByte dv dt idx => dvalue_extract_byte dv dt (Z.of_N idx) *)
  (*                 end) l'). *)
  (*           destruct_err_oom_poison y; inv H1. *)
  (*           forward IHREF; auto. *)
            
  (*           rewrite IHREF in  *)
            
  (*       remember (N.to_nat (SIZEOF.sizeof_dtyp t1)) as n. *)
  (*       clear Heqn. *)
  (*       remember 0 as s. *)
  (*       clear Heqs. *)
  (*       generalize dependent s. *)
  (*       induction n; intros s Heqres. *)
  (*       + cbn in *; inv Heqres. *)
  (*       + cbn in *. *)
  (*         remember (dvalue_extract_byte msg t1 (Z.of_N s)) as extr. *)
  (*         destruct_err_oom_poison extr; cbn in *; inv Heqres. *)
  (*         * remember(map_monad *)
  (*                (fun db : dvalue_byte => *)
  (*                 match db with *)
  (*                 | DVALUE_ExtractByte dv dt idx => dvalue_extract_byte dv dt (Z.of_N idx) *)
  (*                 end) (map (fun idx : N => DVALUE_ExtractByte msg t1 idx) (Nseq (N.succ s) n))) as m. *)
  (*           destruct_err_oom_poison m; inv H0. *)
  (*           eapply IHn; eauto. *)
  (*         * rewrite dvalue_extract_byte_equation in Heqextr. *)
  (*   } *)
  ...
  (* Lemma dvalue_bytes_to_dvalue_no_oom_fin_inf : *)
  (*   forall τ t1 dv_fin msg, *)
  (*     @ErrOOMPoison_handle_poison_and_oom (err_ub_oom_T IdentityMonad.ident) *)
  (*       (@Monad_err_ub_oom IdentityMonad.ident IdentityMonad.Monad_ident) *)
  (*       (@RAISE_ERROR_err_ub_oom IdentityMonad.ident IdentityMonad.Monad_ident) *)
  (*       (@RAISE_OOM_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident) dvalue *)
  (*       DVALUE_Poison (DVALUE_BYTES.dvalue_bytes_to_dvalue (dvalue_to_dvalue_bytes dv_fin t1) τ) = raise_oom msg -> False. *)
  (* Proof. *)
  (*   intros τ t1 dv_fin msg H. *)
  (*   remember ((@DVALUE_BYTES.dvalue_bytes_to_dvalue ErrOOMPoison *)
  (*          (@EitherMonad.Monad_eitherT ERR_MESSAGE (OomableT Poisonable) *)
  (*             (@Monad_OomableT Poisonable MonadPoisonable)) *)
  (*          (@RAISE_ERROR_MonadExc ErrOOMPoison *)
  ...
  (*             (@EitherMonad.MonadT_eitherT ERR_MESSAGE (OomableT Poisonable) *)
  (*                (@Monad_OomableT Poisonable MonadPoisonable)) *)
  (*             (@RAISE_OOMABLE_OomableT Poisonable MonadPoisonable)) (dvalue_to_dvalue_bytes dv_fin t1) *)
  (*          τ)) as res. *)
  (*   destruct_err_oom_poison res; inv H. *)
  (*   - cbn in *; *)
  (*       break_match_hyp_inv. *)
  (*   - cbn. *)
  ...
  Lemma concretize_uvalue_bytes_fin_inf :
    forall uvs_inf uvs_fin res
      ...
      @concretize_uvalue_bytes ErrUbOomProp Monad_ErrUbOomProp
        ...
        (@RAISE_UB_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (@RAISE_OOM_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (fun (A : Type) (x ue : err_ub_oom A) => x = ue) uvs_fin (ret res) ->
      @IS1.LLVM.MEM.CP.CONCBASE.concretize_uvalue_bytes ErrUbOomProp Monad_ErrUbOomProp
        ...
        (@RAISE_UB_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (@RAISE_OOM_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (fun (A : Type) (x ue : err_ub_oom A) => x = ue) uvs_inf (ret (map fin_to_inf_dvalue_byte res)).
  Proof.
    intros uvs_inf uvs_fin res IH REF CONC.
  ...
  Proof.
    intros uvs l dv_fin dt Heqo IH EXTRACT.
  ...
  Lemma concretize_uvalue_bytes_err_fin_inf :
    forall uvs_inf uvs_fin msg
      (IH : forall (u : DV1.uvalue),
          Exists (DV1.uvalue_subterm u) uvs_inf ->
          forall uv_fin : DV2.uvalue,
            uvalue_refine_strict u uv_fin ->
      ...
      @concretize_uvalue_bytes ErrUbOomProp Monad_ErrUbOomProp
        ...
        (@RAISE_UB_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (@RAISE_OOM_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (fun (A : Type) (x ue : err_ub_oom A) => x = ue) uvs_fin (ERR_unERR_UB_OOM msg) ->
      @IS1.LLVM.MEM.CP.CONCBASE.concretize_uvalue_bytes ErrUbOomProp Monad_ErrUbOomProp
        (fun (dt0 : dtyp) (edv : err_ub_oom IS1.LP.Events.DV.dvalue) =>
           match @unERR_UB_OOM IdentityMonad.ident IS1.LP.Events.DV.dvalue edv with
           | {|
        ...
        (@RAISE_UB_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (@RAISE_OOM_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (fun (A : Type) (x ue : err_ub_oom A) => x = ue) uvs_inf (ERR_unERR_UB_OOM msg).
  Proof.
    intros uvs_inf uvs_fin msg IH REF CONC.
    rewrite concretize_uvalue_bytes_equation in CONC.
    rewrite IS1.LLVM.MEM.CP.CONCBASE.concretize_uvalue_bytes_equation.
  ...
  Lemma extractbytes_to_dvalue_err_fin_inf :
    forall uvs l err_msg dt
      (Heqo : map_monad uvalue_convert_strict uvs = NoOom l)
      (IH : forall (u : DV1.uvalue),
          Exists (DV1.uvalue_subterm u) uvs ->
          forall uv_fin : DV2.uvalue,
      ...
      @extractbytes_to_dvalue ErrUbOomProp Monad_ErrUbOomProp
        ...
        (@RAISE_UB_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (@RAISE_OOM_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (fun (A : Type) (x ue : err_ub_oom A) => x = ue) l dt (ERR_unERR_UB_OOM err_msg) ->
      @IS1.LLVM.MEM.CP.CONCBASE.extractbytes_to_dvalue ErrUbOomProp Monad_ErrUbOomProp
        (fun (dt0 : dtyp) (edv : err_ub_oom IS1.LP.Events.DV.dvalue) =>
           match @unERR_UB_OOM IdentityMonad.ident IS1.LP.Events.DV.dvalue edv with
           | {|
        ...
        (@RAISE_OOM_err_ub_oom_T IdentityMonad.ident IdentityMonad.Monad_ident)
        (fun (A : Type) (x ue : err_ub_oom A) => x = ue) uvs dt
        (ERR_unERR_UB_OOM err_msg).
  Proof.
    intros uvs l msg dt Heqo IH EXTRACT.
    rewrite IS1.LLVM.MEM.CP.CONCBASE.extractbytes_to_dvalue_equation.
    rewrite extractbytes_to_dvalue_equation in EXTRACT.
    repeat red in EXTRACT.
    destruct EXTRACT as (?&?&?&?&?).
    ...
    { exists (ERR_unERR_UB_OOM msg).
      exists (fun _ => ERR_unERR_UB_OOM msg).
      split; cbn; eauto.
      eapply concretize_uvalue_bytes_err_fin_inf; eauto.
      eapply map_monad_oom_Forall2; eauto.
    }
  ...
  Proof.
    ...
    - (* UVALUE_Select *)
      ...
      specialize (H1 x1).
      forward H1; [cbn; auto|].
      eapply eval_select_err_fin_inf; eauto.
      { intros ? ?.
        eapply IH; eauto.
        repeat constructor.
      }
    ...
    - (* UVALUE_ConcatBytes *)
      ...
      break_match_hyp.
      2: {
        eapply extractbytes_to_dvalue_err_fin_inf; eauto.
        intros u H0 uv_fin H1 err_msg0 H2.
        eapply IH; eauto.
        eapply DV1.uvalue_concat_bytes_strict_subterm; eauto.
      }
      pose proof (map_monad_oom_length _ _ _ Heqo) as LEN.
      ...
      cbn.
      eapply extractbytes_to_dvalue_err_fin_inf; eauto.
      intros u H uv_fin H0 err_msg0 H1.
      eapply IH; eauto.
      eapply DV1.uvalue_concat_bytes_strict_subterm; eauto.
  Qed.
