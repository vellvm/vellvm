...
Definition Trace := itree (IO +' (failureE +' debugE)).
Hint Unfold Trace.


(* Trace Utilities ---------------------------------------------------------- *)
Definition debug {E : Type -> Type} `{debugE -< E} msg : Trace unit :=
...
Definition raise_Trace := @fail (IO +' (failureE +' debugE)) _.
CoFixpoint catch_Trace {X} (f:string -> Trace X) (t : Trace X) : Trace X :=
  match (observe t) with
  | RetF x => Ret x
  | TauF t => Tau (catch_Trace f t)
  | VisF _ (inr1 (inl1 (Fail s))) k => f s
  | VisF _ (inr1 (inr1 (Debug s))) k => Tau (catch_Trace f t) (* TODO this is probably wrong *)
