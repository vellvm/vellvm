...
| Store  (a:dvalue) (v:dvalue)
| GEP    (t:dtyp) (v:dvalue) (vs:list dvalue)
| ItoP   (i:dvalue)
| PtoI   (a:dvalue)
| Call   (t:dtyp) (f:string) (args:list dvalue)
.    

Definition IO_reaction (io:IO_action) : Type :=
  match io with
  | Store _ _ => unit
  | _ => dvalue
  end.

Canonical Structure IO : Effect :=
  {|
    action := IO_action;
...
Global Instance monad_trace : (@Monad Trace) functor_trace :=
  { mret X x := Ret (inr x);
    mbind := fun {A B} (X:Trace A) (f : A -> Trace B) => ITree.bind X
                                                                     (fun m => match m with
                                                                            | inl s => Ret (inl s)
                                                                            | inr x => f x
                                                                            end)
