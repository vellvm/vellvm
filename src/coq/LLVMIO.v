...
Definition LLVM E := itree (CallE +' Locals +' IO +' E).
Hint Unfold LLVM.
...
Definition into_debug {E F} (h : E ~> LLVM F) : CallE +' Locals +' IO +' (F +' E) ~> LLVM F :=
  fun x e =>
    match e with
    | inr1 (inr1 (inr1 (inr1 e))) => h _ e
...
Definition raise_LLVM {E} := @fail (CallE +' Locals +' IO +' (failureE +' E)) _.
CoFixpoint catch_LLVM {E} {X}
  ...
  | VisF _ (inr1 (inr1 (inr1 (inl1 (Fail s))))) k => f s
