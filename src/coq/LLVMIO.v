...
Lemma lift_err_ret : forall (A:Type), pointwise_relation _ eutt (lift_err (fun (x:A) => ret x))  (fun x => Ret x).
Proof.
  do 2 intro.
  unfold lift_err.
...
Lemma ret_of_bind : forall A (t:Trace A),
    bind t (fun x => ret x) ≅ t.
Proof.
  intros A t.
  cbn. unfold bind_trace.
  change (fun x => Ret (inr x)) with (fun (x:A) => ret x).
  SearchAbout Proper ITree.bind.
  eapply eq_itree_bind.
  
  rewrite lift_err_ret.
  intros A. pcofix CIH.
  intros t.
  pfold.
  cbn. unfold bind_trace.
