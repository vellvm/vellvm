...
Section interp_prop_oom.
  ...
  | Interp_Prop_OomT_Ret : forall r1 r2 (REL: RR r1 r2),
      interp_prop_oomTF b1 b2 sim (RetF r1) (RetF r2)

  | Interp_Prop_OomT_Tau : forall t1 t2 (HS: sim t1 t2),
      interp_prop_oomTF b1 b2 sim (TauF t1) (TauF t2)

  | Interp_Prop_OomT_TauL : forall t1 t2
                          (CHECK: is_true b1)
                          (HS: interp_prop_oomTF b1 b2 sim (observe t1) t2),
      interp_prop_oomTF b1 b2 sim (TauF t1) t2

  | Interp_Prop_OomT_TauR : forall t1 t2
                          (CHECK: is_true b2)
                          (HS: interp_prop_oomTF b1 b2 sim t1 (observe t2)),
      interp_prop_oomTF b1 b2 sim t1 (TauF t2)

  | Interp_Prop_OomT_Vis_OOM : forall A (e : OOM A) k1 t1 t2
                         (HT1: t1 ≅ vis e k1),
      interp_prop_oomTF b1 b2 sim (observe t1) t2

  | Interp_Prop_OomT_Vis : forall A e k1 k2 (ta t2 : itree F _)
                  (HK : forall (a : A), Returns a ta -> sim (k1 a) (k2 a)),
        h_spec _ e ta ->
        t2 ≈ ta >>= k2 ->
        interp_prop_oomTF b1 b2 sim (VisF e k1) (observe t2).

  Hint Constructors interp_prop_oomTF : core.

  Lemma interp_prop_oomTF_mono b1 b2 x0 x1 sim sim'
        (IN: interp_prop_oomTF b1 b2 sim x0 x1)
        (LE: sim <2= sim'):
    interp_prop_oomTF b1 b2 sim' x0 x1.
  Proof.
  ...
  Definition interp_prop_oomT_ b1 b2 sim (t0 : itree _ R1) (t1 : itree _ R2) :=
    interp_prop_oomTF b1 b2 sim (observe t0) (observe t1).
  Hint Unfold interp_prop_oomT_ : core.

  Lemma interp_prop_oomT__mono b1 b2 : monotone2 (interp_prop_oomT_ b1 b2).
  Proof.
  ...
  (* Definition 5.2 *)
  Definition interp_prop_oom' b1 b2 :
    itree _ R1 -> PropT _ R2 :=
    paco2 (interp_prop_oomT_ b1 b2) bot2.

  Definition interp_prop_oom :
    itree _ R1 -> PropT _ R2 :=
    interp_prop_oom' true true.
  ...
  #[global] Instance interp_prop_oom_eutt_Proper_impl_ :
    forall (x : _ R1), Proper (eutt eq ==> impl) (interp_prop_oom x).
  Proof.
    ...
    - rewrite <- Heqi.
      ...
      + inv INR; try (exfalso; eapply EQ; eauto; fail).
        ...
        * eapply IHREL; eauto. pstep_reverse.
          assert (interp_prop_oom t0 (Tau t1)) by (pstep; auto).
  ...
  #[global] Instance interp_prop_oom_eutt_Proper_impl :
    Proper (eutt eq ==> eutt eq ==> impl) (interp_prop_oom).
  Proof.
      ...
      + inv INR; try (exfalso; eapply EQ; eauto; fail).
        ...
        * remember (VisF e k1) as ot.
          ...
          -- pose proof @Interp_Prop_OomT_Vis.
              ...
              subst. cbn in *. inv H0.
              match goal with
              | |- interp_prop_oomTF _ _ _  ?l _ => change l with (observe (go l))
              end.
              eapply Interp_Prop_OomT_Vis_OOM; reflexivity.
          -- inv Heqot.
             ...
             eapply CIH; eauto.
              ...
              punfold HK.
        * eapply IHREL; eauto. pstep_reverse.
          assert (interp_prop_oom (Tau t0) t2) by (pstep; auto).
  ...
  #[global] Instance interp_prop_oom_eutt_Proper :
    Proper (eutt eq ==> eutt eq ==> iff) interp_prop_oom.
  Proof.
    split; intros; [rewrite <- H, <- H0 | rewrite H, H0]; auto.
  Qed.
...
Arguments interp_prop_oom {_ _ _ _} _ {_ _}.
Arguments interp_prop_oom' {_ _ _ _} _ {_ _}.
...
#[global] Instance interp_prop_oom_Proper_eq :
  forall (E F OOM : Type -> Type) (h_spec : forall T : Type, E T -> PropT F T)
    ...
    Proper (@eutt _ _ _ RR ==> eq ==> flip Basics.impl) (@interp_prop_oom E F OOM _ h_spec _ _ RR).
Proof.
    ...
    + inv INR; try (exfalso; eapply EQ; eauto; fail).
      ...
      * remember (VisF e k2) as ot.
        ...
        -- pose proof @Interp_Prop_OomT_Vis.
            ...
            subst. cbn in *. inv H0.
            match goal with
            | |- interp_prop_oomTF _ _ _ _ _  ?l _ => change l with (observe (go l))
            end.
            eapply Interp_Prop_OomT_Vis_OOM; reflexivity.
        -- inv Heqot.
            dependent destruction H3. econstructor.
            ...
            punfold HK.
      * eapply IHREL; eauto. pstep_reverse.
        assert (interp_prop_oom h_spec RR (Tau t2) t0 (OOME:=CONSTRAINT)) by (pstep; auto).
        apply interp_prop_oom_inv_tau_l in H. punfold H.
  ...
  - rewrite <- Heqi0. rewrite itree_eta in HT1. rewrite <- Heqi.
    ...
    destruct H as (?&?). cbn in *. inv H.
      ...
      eapply Interp_Prop_OomT_Vis_OOM; subst. reflexivity.
  - rewrite Heqi in EQ.
    remember (VisF e k1).
    hinduction EQ before CIH; intros; try inversion Heqi1; pclearbot; inv Heqi.
    + dependent destruction H3.
...
Section interp_prop_oom_extra.
  ...
  Lemma interp_prop_oom_clo_bind {U} t1 t2 k1 k2
        (EQT: @interp_prop_oom E F OOM _ h U _ eq t1 t2)
        (EQK: forall u1 u2, eq u1 u2 -> @interp_prop_oom E F OOM _ h _ _ eq (k1 u1) (k2 u2)):
    @interp_prop_oom E F OOM _ h _ _ eq (ITree.bind t1 k1) (ITree.bind (U := U) t2 k2).
  Proof.
    revert_until U.
  ...
  Proof.
    ...
    - subst.
      ...
      + dependent destruction H2.
        pclearbot. pstep.
        eapply Interp_Prop_OomT_Vis_OOM; eauto.
        reflexivity.
