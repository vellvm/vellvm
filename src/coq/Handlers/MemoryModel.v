...
Module Type MemoryModelSpecPrimitives (LP : PARAMS). 
  Import LP.Addr.
  Import LP.Sizeof.
  Import LP.ByteImpl.
  Import LP.IP.
...
Module Type _SERIALIZE (LP : PARAMS).
  Import LP.DV.
  Import LP.Sizeof.
  Import LP.ByteImpl.
  ...
  (*   List.map Byte (bytes_of_int e count z). *)
    ...
    (* TODO: Handle Alignment - see the git archive for sketches *)
    ...
    Parameter re_sid_ubytes_helper : forall {M} `{Monad M} `{MonadStoreId M} `{RAISE_ERROR M}
            (bytes : list (N * SByte)) (acc : NMap SByte), M (NMap SByte).

    Parameter re_sid_ubytes_helper_equation : forall {M} `{Monad M} `{MonadStoreId M} `{RAISE_ERROR M}
...
Module _ADD_SERIALIZE (LP : PARAMS) : _SERIALIZE LP.
  Import LP.
  Import DV.
  Import Sizeof.
  Import ByteImpl.
...
Module Type MemoryModelSpecPrimitives_SERIALIZE (P:PARAMS) := MemoryModelSpecPrimitives P <+ _SERIALIZE P.

(*
Module Type _PARAMS_SPEC (P:PARAMS).
  Declare Module SPEC : MemoryModelSpecPrimitives_SERIALIZE P.
End _PARAMS_SPEC.

Module Type PARAMS_SPEC := PARAMS <+ _PARAMS_SPEC.
*)
...
Module Type MemoryModelSpec (LP : PARAMS) (MMSP : MemoryModelSpecPrimitives_SERIALIZE LP). 
  Import LP.
  Import Addr.
  Import Sizeof.
  Import DV.
  ...
  Section Handlers.
    Definition handle_memory_prop : (MemoryE dtyp uvalue dvalue) ~> MemPropT MemState
      := fun T m =>
           match m with
           (* Unimplemented *)
           | MemPush =>
    ...
    Definition handle_intrinsic_prop : (IntrinsicE dtyp dvalue) ~> MemPropT MemState
      := fun T e =>
           match e with
...
Module Type MemoryModelSpecFull (LP : PARAMS) := (MemoryModelSpecPrimitives_SERIALIZE LP) <+ (MemoryModelSpec LP).

(*
Module MakeMemoryModelSpec (LP : PARAMS) (MMSP : MemoryModelSpecPrimitives_SERIALIZE LP) <: MemoryModelSpec LP MMSP.
  Include MemoryModelSpec LP MMSP.
End MakeMemoryModelSpec.

Module MakeMemoryModelSpecFull (LP : PARAMS) (MMSP : MemoryModelSpecPrimitives_SERIALIZE LP) <: MemoryModelSpecFull LP.
  Include MMSP.
  Include MemoryModelSpec LP MMSP.
End MakeMemoryModelSpecFull.
*)
...
Module Type MemoryExecMonad (LP : PARAMS) (MMSP : MemoryModelSpecFull LP).
  Import EitherMonad.
  Import Monad.
...
Module Type MMEF (LP : PARAMS) := (MemoryModelSpecFull LP) <+ (MemoryExecMonad LP).

(* Module MakeMemoryExecMonad (LP : PARAMS) (MMSP : MemoryModelSpecFull LP) <: MemoryExecMonad LP MMSP. *)
(*   Include MemoryExecMonad LP MMSP. *)
(* End MakeMemoryExecMonad. *)

Module Type MemoryModelExecPrimitives (LP : PARAMS) (M : MMEF LP).
  Import LP.
  Import Addr.
  Import Sizeof.
  Import ByteImpl.
...
Module Type MMEP (LP : PARAMS) := (MMEF LP) <+ (MemoryModelExecPrimitives LP).


Module Type MemoryModelExec (LP : PARAMS) (M : MMEP LP).
  Import LP.
  Import Addr.
  Import Sizeof.
  Import DV.
  ...
  Section Handlers.
    ...
    Definition handle_memory `{MemMonad ExtraState MemM (itree Eff)} : (MemoryE dtyp uvalue dvalue) ~> MemM
      := fun T m =>
           match m with
           (* Unimplemented *)
           | MemPush =>
    ...
    Definition handle_intrinsic `{MemMonad ExtraState MemM (itree Eff)} : (IntrinsicE dtyp dvalue) ~> MemM
      := fun T e =>
           match e with
           | Intrinsic t name args =>
               (* Pick all arguments, they should all be unique. *)
...
Module Type MMME (LP : PARAMS) := (MMEP LP) <+ MemoryModelExec LP.


Module MakeMemoryModelExec (LP : PARAMS) (MMEP : MMEP LP) <: MMME LP.
  Include MMEP.
  Include MemoryModelExec LP MMEP.
End MakeMemoryModelExec.



Module MemoryHelpers (LP : PARAMS) (MMSP : MemoryModelSpecPrimitives LP).
  (*** Other helpers *)
  Import LP.
  Import GEP.
  Import Addr.
  ...
  Proof.
    ...
    - cbn in *.
      ...
      destruct (map_monad
                  (fun ix : IP.intptr =>
                     GEP.handle_gep_addr (DTYPE_I 8) ptr [DV.DVALUE_IPTR ix])
                  l) eqn:HMAPM.
      + cbn in CONSEC.
        setoid_rewrite rbm_raise_bind in CONSEC; auto.
      ...
      (forall p ix_nat,
          ...
          exists ix,
            NoOom ix = IP.from_Z (Z.of_nat ix_nat) /\
              handle_gep_addr (DTYPE_I 8) ptr [DVALUE_IPTR ix] = inr (ret p)).
  Proof.
    ...
    setoid_rewrite bind_ret_l in CONSEC.
    destruct (map_monad
                (fun ix : IP.intptr => handle_gep_addr (DTYPE_I 8) ptr [DVALUE_IPTR ix]) l) eqn:MAP.
    { cbn in CONSEC.
      setoid_rewrite rbm_raise_bind in CONSEC; auto.
      apply rw_ret_nin_raise in CONSEC; try contradiction; auto.
    }
  ...
  Proof.
    ...
    - pose proof (map_monad_err_succeeds
                    (fun ix : IP.intptr => handle_gep_addr (DTYPE_I 8) ptr [DVALUE_IPTR ix]) l) as HMAPM.
      forward HMAPM.
      { intros a IN.
...
Module MemoryModelTheory (LP : PARAMS) (M : MMME LP).
  Import M.
  Import LP.DV.
  Import LP.OVER.
  Include (MemoryHelpers LP M).
  ...
  Section Correctness.
    ...
    Proof.
      ...
      - cbn. repeat rewrite re_sid_ubytes_helper_equation. apply exec_correct_ret.
      - cbn. repeat rewrite re_sid_ubytes_helper_equation.
    ...
    Proof.
      ...
      unfold no_overlap.
      unfold no_overlap.
      break_match; [|eapply exec_correct_raise_ub].
      apply exec_correct_bind.
      apply read_bytes_correct.
      intros * RUN.
      apply write_bytes_correct.
    ...
    Lemma handle_memory_correct :
      forall T (m : MemoryE dtyp uvalue dvalue T) pre,
    ...
    Lemma handle_intrinsic_correct:
      forall T (e : IntrinsicE dtyp dvalue T) pre,
...
Module MemStateInfiniteHelpers (LP : PARAMS_BIG) (ME : MMEF LP). 
  (* Intptrs are "big" *)
  Import LP.
  Import DV.
  Import Addr.
  ...
  Proof.
    ...
    { apply map_monad_OOM_fail in HSEQUENCE as [a [IN EQA]].
      ...
      pose proof
        (int_to_ptr_safe (ptr_to_int ptr + Z.of_N (sizeof_dtyp (DTYPE_I 8)) * to_Z ix)
...
Module Type MemoryModelInfiniteSpec (LP : PARAMS_BIG) (M : MMME LP).
  (* Intptrs are "big" *)
  Import LP.
  Import DV.
  Import IP.
...
Module Type MMIE (LP : PARAMS_BIG) := (MMME LP) <+ (MemoryModelInfiniteSpec LP).

Module MemoryModelInfiniteSpecHelpers (LP : PARAMS_BIG) (M : MMIE LP).
  Import LP.
  Import DV.
  Import IP.
  Import Addr.
