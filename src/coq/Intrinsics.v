...
Module Make(A:MemoryAddress.ADDRESS)(LLVMIO: LLVM_INTERACTIONS(A)).
  ...
  Definition handle_intrinsics (intrinsic_defs : intrinsic_definitions)
    : (IO +' (failureE +' debugE)) ~> itree (IO +' (failureE +' debugE)) :=
    ...
    fun X (e : (IO +' (failureE +' debugE)) X) =>
      match e return itree (IO +' (failureE +' debugE)) X with
      | inl1 e' =>
        match e' in IO Y return X = Y -> itree (IO +' (failureE +' debugE)) Y with
        | Call _ fname args => 
          ...
          | None => fun pf => (eq_rect X (fun a => itree (IO +' (failureE +' debugE)) a) (ITree.liftE e)) dvalue pf
          end
        | Store _ _ => fun pf  => (eq_rect X (fun a => itree (IO +' (failureE +' debugE)) a) (ITree.liftE e)) unit pf
        | _ => fun pf  => (eq_rect X (fun a => itree (IO +' (failureE +' debugE)) a) (ITree.liftE e)) dvalue pf
        end eq_refl
