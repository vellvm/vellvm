...
(* TODO: May not need this in ctrees anymore *)
...
(*     (* M may be PropT or itree... *) *)
(*     Definition MemStateFreshT_run {A} {Eff B} `{FailureE -< Eff} `{OOME -< Eff} `{UBE -< Eff} (ma : MemStateFreshT (ctree Eff B) A) (ms : MemState) (st : MemStateFreshT_State) : ctree Eff B (MemStateFreshT_State * (MemState * A)). *)
(*     Proof using. *)
(*       unfold MemStateFreshT in *. *)
(*       specialize (ma st ms). *)
(*       unfold MemStateFreshT_State. *)
...
(*     Proof using. *)
(*       esplit with *)
(*         (MemMonad_run := fun A => @MemStateFreshT_run A Effout MB _ _ _); try solve [typeclasses eauto]. *)
