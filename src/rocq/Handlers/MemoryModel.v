...
Module MemoryModelInfiniteSpecHelpers (LP : LLVMParamsBig) (MP : MEMORY_PARAMS LP) (MMSP : MemoryModelSpecPrimitives LP MP) (MMS : MemoryModelSpec LP MP MMSP) (MMIS : MemoryModelInfiniteSpec LP MP MMSP MMS).
  ...
  Lemma generate_num_poison_bytes_h_cons :
    forall dt len start bytes,
      generate_num_poison_bytes_h (N.succ start) len dt = NoOom bytes ->
      generate_num_poison_bytes_h start (N.succ len) dt =
        b <- generate_num_poison_bytes_h start 1 dt;;
        rest <- generate_num_poison_bytes_h (N.succ start) len dt;;
        NoOom (b ++ rest).
  Proof.
    intros dt len start bytes NOOM.
    cbn.
    unfold generate_num_poison_bytes_h in *.
    match goal with
  ...
  (* TODO: should this be here? *)
  Lemma generate_num_poison_bytes_h_succeeds :
    forall dt start,
    exists bytes : list memory_byte, generate_num_poison_bytes_h start (sizeof_dtyp dt) dt = ret bytes.
  Proof.
    intros dt.
    ...
    - specialize (IHn (N.succ start)).
      ...
      eexists.
      erewrite generate_num_poison_bytes_h_cons; eauto.
      rewrite IHn.
      cbn.
      reflexivity.
  Qed.
  ...
  (* TODO: should this be here? *)
  Lemma generate_num_poison_bytes_succeeds :
    forall dt,
    exists bytes : list memory_byte, generate_num_poison_bytes (sizeof_dtyp dt) dt = ret bytes.
  Proof.
    intros dt.
    eapply generate_num_poison_bytes_h_succeeds.
  Qed.

  (* TODO: should this be here? *)
  Lemma generate_poison_bytes_succeeds :
    forall dt,
    exists bytes, generate_poison_bytes dt = ret bytes.
  Proof.
    intros dt.
    unfold generate_poison_bytes.
    apply generate_num_poison_bytes_succeeds.
  Qed.
  ...
  (*   forall (ms_init ms_fresh_pr : MemState) dt num_elements pr *)
  ...
  (* Proof. *)
  (*   intros ms_init ms_fresh_pr dt num_elements pr sid FRESH_SID FRESH_PR NON_VOID. *)
