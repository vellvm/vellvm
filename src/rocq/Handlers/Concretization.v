...
Module Type ConcretizationBase (LP : LLVMParams) (MP : MEMORY_PARAMS LP) (Byte : ByteModule LP MP.BYTE_IMPL).
  ...
  (*     concretize_uvalueM M undef_handler ERR_M lift_ue u = *)
    ...
    Definition eval_select
      (M : Type -> Type) `{Monad M} (undef_handler : dtyp -> M dvalue)
...
Module MakeBase (LP : LLVMParams) (MP : MEMORY_PARAMS LP) (Byte : ByteModule LP MP.BYTE_IMPL) <: ConcretizationBase LP MP Byte.
  ...
  Export Byte.
  Include ConcretizationBase LP MP Byte
    with Definition ptr_size := (8%nat)
    with Definition endianess := ENDIAN_LITTLE.
      ...
      := match uvs with
         ...
         | (uv::uvs) => 
             ...
             | UVALUE_ExtractByte byte_uv dt idx sid => 
                 ...
                 | None =>
                     (* Concretize the uvalue *)
                     dv <- @concretize_uvalueM M HM undef_handler ERR_M HM_ERR ERR UB OOM lift_ue byte_uv;; 
                     let dv_byte := DVALUE_ExtractByte dv dt idx in
                     let acc := new_concretized_byte acc byte_uv dv sid in
                     (* Concretize the rest of the bytes *)
                     rest <- concretize_uvalue_bytes_helper acc uvs;;
