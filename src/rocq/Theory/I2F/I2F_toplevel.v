...
Proof.
  ...
  destruct Hb as [p1 p2 Hp | sz i | p1 p2 Hp | d | f | | | sz bits1 bits2 Hbits]; try (rbind (fun _ _ => False); [apply ruttc_trigger_cast; cbnn; simp I2FE_UB; auto | intros ?? []]); [].
  ...
  - intros iptr1 iptr2 Hiptr.
    ...
    + intros addr1 addr2 Haddr.
      ...
      * intros d1 d2 Hd.
        ...
        destruct Hb as [q1 q2 Hq | sz i | q1 q2 Hq | dd | ff | | | sz bits1 bits2 Hbits];
          try (unfold raise; apply ruttc_trigger_cast; cbnn; simp I2FE_Failure; auto); [].
...
Proof.
  ...
  destruct Hu as [b1 b2 Hb | | ];
    try (rbind (fun _ _ => False); [apply ruttc_trigger_cast; cbnn; simp I2FE_UB; auto | intros ?? []]); [].
  destruct Hb as [p1 p2 Hp | sz i | p1 p2 Hp | d | f | | | sz bits1 bits2 Hbits];
    try (rbind (fun _ _ => False); [apply ruttc_trigger_cast; cbnn; simp I2FE_UB; auto | intros ?? []]); [].
